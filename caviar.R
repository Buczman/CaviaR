...
caviarOptimisation <- function(data, model, pval, REP = 5, MAXITER = 500, seed = 1){
  ...
  VaR <- RQObjectiveFunction(bestPar, 2, model, data, obs, pval, emp_qnt)
...
RQObjectiveFunction <- function(beta,out,model,data,obs,pval,emp_qnt){
  VaR = rep(0,obs)
  Hit = VaR
  ...
  if (model == 1){
    ...
    VaR <- caviar_SAV(beta, data, VaR[1], VaR, obs)
    Hit <- (data < -VaR) - pval

  }else if (model == 2){
    
    VaR <- caviar_AS(beta, data, VaR[1], VaR, obs)
    Hit <- (data < -VaR) - pval
    
  }else if (model == 3){
    
    VaR <- caviar_GARCH(beta, data, VaR[1], VaR, obs)
    Hit <- (data < -VaR) - pval
    
  }else if (model == 4){
    
    k = 5
    VaR <- caviar_ADAPTIVE(pval, k, beta, data, VaR[1], VaR, obs)
    Hit <- (data < -VaR) - pval
